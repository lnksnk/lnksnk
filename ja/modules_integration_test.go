package ja_test // this is on purpose in a separate package
...
type simpleComboResolver struct {
	...
	reverseCache map[ja.ModuleRecord]string
	fs           fs.FS
	custom       func(interface{}, string) (ja.ModuleRecord, error)
}
...
func newSimpleComboResolver() *simpleComboResolver {
	return &simpleComboResolver{cache: make(map[string]cacheElement), reverseCache: make(map[ja.ModuleRecord]string)}
}

func (s *simpleComboResolver) resolve(referencingScriptOrModule interface{}, specifier string) (ja.ModuleRecord, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	p, err := ja.ParseModule(specifier, string(b), s.resolve)
	if err != nil {
		s.cache[specifier] = cacheElement{err: err}
		return nil, err
	}
...
func TestNotSourceModulesBigTest(t *testing.T) {
	...
	resolver.custom = func(_ interface{}, specifier string) (ja.ModuleRecord, error) {
		switch specifier {
	...
	if promise.State() != ja.PromiseStateFulfilled {
...
func TestNotSourceModulesBigTestDynamicImport(t *testing.T) {
	...
	vm.SetPromiseRejectionTracker(func(p *ja.Promise, operation ja.PromiseRejectionOperation) {
		t.Fatal(p.Result())
	})
	vm.SetImportModuleDynamically(func(referencingScriptOrModule interface{}, specifierValue ja.Value, promiseCapability interface{}) {
...
var _ ja.ModuleRecord = &simpleModuleImpl{}
...
func (s *simpleModuleImpl) ResolveExport(exportName string, resolveset ...ja.ResolveSetElement) (*ja.ResolvedBinding, bool) {
	if exportName == "coolStuff" {
		return &ja.ResolvedBinding{
...
func (s *simpleModuleImpl) Evaluate(rt *ja.Runtime) *ja.Promise {
	p, res, _ := rt.NewPromise()
	res(&simpleModuleInstanceImpl{rt: rt})
	return p
}

func (s *simpleModuleImpl) GetExportedNames(callback func([]string), records ...ja.ModuleRecord) bool {
...
func (si *simpleModuleInstanceImpl) GetBindingValue(exportName string) ja.Value {
	if exportName == "coolStuff" {
...
type cyclicModuleImpl struct {
	...
	exports          map[string]unresolvedBinding
	resolve          ja.HostResolveImportedModuleFunc
}

var _ ja.CyclicModuleRecord = &cyclicModuleImpl{}

func (s *cyclicModuleImpl) InitializeEnvironment() error {
	return nil
}

func (s *cyclicModuleImpl) Instantiate(_ *ja.Runtime) (ja.CyclicModuleInstance, error) {
...
func (s *cyclicModuleImpl) Evaluate(rt *ja.Runtime) *ja.Promise {
	return rt.CyclicModuleRecordEvaluate(s, s.resolve)
}

func (s *cyclicModuleImpl) ResolveExport(exportName string, resolveset ...ja.ResolveSetElement) (*ja.ResolvedBinding, bool) {
...
func (s *cyclicModuleImpl) GetExportedNames(callback func([]string), records ...ja.ModuleRecord) bool {
...
func (si *cyclicModuleInstanceImpl) ExecuteModule(rt *ja.Runtime, _, _ func(interface{}) error) (ja.CyclicModuleInstance, error) {
	si.rt = rt
	return si, nil
}

func (si *cyclicModuleInstanceImpl) GetBindingValue(exportName string) ja.Value {
