...
import (
	...
	"github.com/lnksnk/lnksnk/es/token"
...
func (c *compiler) compileModule(module *SourceTextModuleRecord) {
	...
	c.compileDeclList(false, in.DeclarationList...)
	...
	c.compileStatements(true, in.Body...)
	c.emit(loadUndef)
	c.emit(ret)
	if enter != nil {
...
func (c *compiler) compileDeclList(inFunc bool, v ...*ast.VariableDeclaration) {
	for _, value := range v {
