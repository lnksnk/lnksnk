...
var IOSessionHandler sessioning.SessionIOFunc

func init() {

	MAINSESSION = sessioning.NewSession(nil, globalfs.FSYS)
	SESSIONS = MAINSESSION.Sessions()
	IOSessionHandler = sessioning.SessionIOFunc(func(path string, in io.Reader, out io.Writer) {
		ssn := IOSessionHandler.Session(path, MAINSESSION, out, in, globalfs.FSYS)
			...
			if actv {
				if ssn != nil {
					if rqfi = active.ProcessActiveFile(
						...
						out,
						nil,
						ssn.API().RunProgram); rqfi != nil && out != nil {
						if in != nil {
